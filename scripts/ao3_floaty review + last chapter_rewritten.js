// ==UserScript==
// @name         AO3 Floaty Review Box + Last Chapter Shortcut
// @namespace    saxamaphone, CertifiedDiplodocus
// @version      2.4
...
// @grant        none
// @downloadURL  https://update.greasyfork.org/scripts/421689/AO3%20Review%20%2B%20Last%20Chapter%20Shortcut%20%2B%20Kudos-sortable%20Bookmarks.user.js
// @updateURL    https://update.greasyfork.org/scripts/421689/AO3%20Review%20%2B%20Last%20Chapter%20Shortcut%20%2B%20Kudos-sortable%20Bookmarks.meta.js
// ==/UserScript==

/* eslint-env jquery */ //           allows jQuery

// IMPORTANT: for personal use only. Not to be published.

// TODO reposition buttons, button margins
...
(function (jQuery) {
    ...
    function getStoryId() {
        let aMatch = window.location.pathname.match(/works\/(\d+)/)
        if (aMatch !== null) {
            return aMatch[1]
        } else {
    ...
    // Process bookmarks first because of extra sorting steps. Once this is done, handle everything else
        let oBookmarksProcessed = jQuery.Deferred()
        ...
        // Wait to handle the bookmarks after they're loaded
            let oBookmarksLoaded = jQuery.Deferred()

            let bKudos = false, bComplete = false
                ...
                jQuery.when(oBookmarksLoaded).done(function () {
                    ...
                    if (bComplete) {
                        jQuery.each(aBookmarks, function (iArrayIndex) {
                            let sChapters = jQuery(this).find('dd.chapters').html()
                            if (sChapters !== undefined) {
                                let aChapters = sChapters.split('\/')
                    ...
                    let iPage = getURLParameter('page')
                    if (iPage === null) {
                        iPage = 1
                    }
                    ...
                    let iIndex
                    let iNumBookmarks = aBookmarks.length
                    ...
                    if (bComplete) {
                        let sPrevHeading = jQuery('h2.heading').html()
                        jQuery('h2.heading').html(sPrevHeading.replace(/\d+ - \d+ of \d+/, (iPage - 1) * 20 + 1 + ' - ' + iIndex + ' of ' + aBookmarks.length))

                        // Repaginate if necessary
                        let iFinalPage = jQuery('ol.pagination').first().find('li').not('.previous, .next').last().text()
                        let iNewFinalPage = Math.ceil(iNumBookmarks / 20)
                        ...
                        // Rules for AO3 pagination are way too complicated for me to bother replicating, so just going to remove extra pages
                            let aPageLinks = jQuery('ol.pagination').first().find('li')
        ...
        // If not a story page, presume an index page (tags, collections, author, bookmarks, series) and process each work individually
            ...
            // Near as I can figure, the best way of identifying actual stories in an index page is with the h4 tag with class 'heading' within a list of type 'header'
                jQuery('.header h4.heading').each(function () {
                    let sStoryPath = jQuery(this).find('a').first().attr('href')
                    let oHeader = this

                    // If link is from collections, get proper link
                    let aMatch = sStoryPath.match(/works\/(\d+)/)
                    if (aMatch !== null) {
                        let iStoryId = aMatch[1]

                        jQuery.get('/works/' + iStoryId + '/navigate', function (oData) {
                            let sLastChapterPath = jQuery(oData).find('ol li').last().find('a').attr('href')
            ...
            // Include x button to close box
                let sHtml = '<p class="close actions" id="close_floaty"><a aria-label="cancel" style="display: inline-block;">�</a></p>'
                ...
                jQuery('<div/>', {
                    id: 'reviewTextArea',
                    width: 270, // Change for dimensions
                ...
                jQuery('#insert_floaty_text').click(function () {
                    let sInitialText = jQuery('#floaty_textarea').val()
                    let iPosition = jQuery('#floaty_textarea').getCursorPosition()

                    let sHighlightedText = window.getSelection().toString()

                    let sNewText = sInitialText.substr(0, iPosition) + '<i>' + sHighlightedText + '</i>\n' + sInitialText.substr(iPosition)
                ...
                let sReviewTipsHtml = '<p class="close actions" id="close_review_tips"><a aria-label="cancel" style="display: inline-block;">�</a></p>'
