// ==UserScript==
// @name         ao3 secondary char&pairing filter jQUERY
...
(function($) {
    ...
    let badValues = {relationships:[], characters:[]}
    ...
    for (let i=0; i < $works.length; i++) {
        ...
        const $tags = $works.eq(i).find('ul.tags') // [i] gives the native DOM object, not jQuery

        if (checkRels) {
            let firstNrels = $tags.find('.relationships')
            .slice(0, relpad)
        ...
        if (checkChars) {
            let firstNchars = $tags.find('.characters')
            .slice(0, charpad)
            .map (tag => tag.textContent);
        ...
        $works.eq(i).addClass('hiddenwork');
        ...
        button.addEventListener(
            'click', function() {
                $works.eq(i).removeClass('hiddenwork');
                note.remove();
            });
        ...
        $works.eq(i).after(fragment)

    }

    $('<style title="char&pairing filter">').text(`
        .hidereasons {
